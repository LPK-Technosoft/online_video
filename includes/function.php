...
//   $data= mysqli_query($mysqli, "INSERT INTO $table(".implode(",",$fields).") VALUES ('".implode("','", $values )."');") or die( mysqli_error($mysqli) );
//   print_r($data);
}
...
function calculate_time_span($post_time, $flag = false) {
    if ($post_time != '') {
        ...
        if ($seconds < 60)
            ...
            $time = ($mins == 1) ? $mins . " min ago" : $mins . " mins ago";
...
function checkSignSalt($data_info) {
    ...
    if ($data_arr['package_name'] == PACKAGE_NAME) {
        ...
        $set['ONLINE_RADIO'][] = array("success" => -1, "msg" => "Invalid package name");
        header('Content-Type: application/json; charset=utf-8');
        echo $val = str_replace('\\/', '/', json_encode($set, JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT));
        exit();
    }
